...
import sys
import numpy as np
from game_logic import create_game, is_valid, make_move, has_won
...
class GomokuGame:
    def __init__(self):
        ...
        self.game = create_game(BOARD_SIZE).astype(np.int8)
    ...
    def process_click(self, pos):
        ...
        if is_valid(self.game, row, col):
            ...
            # won?
            if has_won(self.game, 5, self.current_player):
                self.game_over = True
    ...
    def restart_game(self):
        """reset"""
        self.game = create_game(BOARD_SIZE).astype(np.int8) # gl? 
        self.current_player = 1
        self.game_over = False
        self.winner = None
        self.last_move = None
